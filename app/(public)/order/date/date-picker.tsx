    ...
    <main className="flex flex-1 flex-col gap-5 pb-32 pt-6">
      ...
      <div className="px-[var(--screen-pad)]">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-[13px] font-semibold">{monthLabel(cursor)}</span>
          <button
            type="button"
            disabled={!canNext}
            onClick={() => setCursor(shiftMonth(cursor, 1))}
        ...
        <div className="grid grid-cols-7 gap-[3px]">
          {WEEKDAYS.map((w, i) => (
            <span key={i} className="pb-1.5 text-center text-[9.5px] text-muted">
              {w}
            </span>
          ))}
          {cells.map((cell, i) =>
...
function DayCell({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "flex aspect-square items-center justify-center text-[12px] tabular-nums",
        disabled && "border border-dashed border-hairline text-muted opacity-50",
      ...
      style={
        selected
          ? {
              background: "var(--wine-fg)",
              color: "#fdf3ee",
              borderColor: "var(--wine-fg)",
            }
          : undefined
      }
    >
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[10.5px] text-muted">
      <span className="flex items-center gap-1.5">
        <span className="h-[7px] w-[7px] border border-hairline" /> Open
      </span>
      <span className="flex items-center gap-1.5">
        <span className="h-[7px] w-[7px]" style={{ background: "var(--wine-fg)" }} />
        Selected
      </span>
      <span className="flex items-center gap-1.5">
        <span className="h-[7px] w-[7px] border border-dashed border-hairline" />
        Too soon
      </span>
    </div>
  );
