...
export function DetailsForm({ sizes, addons }: { sizes: Size[]; addons: Addon[] }) {
  ...
  function toggleAddon(id: string) {
    const next = draft.addon_ids.includes(id)
      ? draft.addon_ids.filter((a) => a !== id)
      : [...draft.addon_ids, id];
    update({ addon_ids: next });
  }

  return (
    <main className="flex flex-1 flex-col gap-8 px-[var(--screen-pad)] pb-36 pt-6">
      <div>
      ...
      <Field label="Size">
        <div className="flex flex-col">
          {sizes.map((s) => (
            <OptionRow
              key={s.id}
              selected={draft.size === s.label}
              onClick={() => update({ size: s.label })}
            >
              {s.label} — {formatPriceRange({ minCents: s.base_price_cents, maxCents: s.base_price_cents })}
            </OptionRow>
          ))}
          {sizes.length === 0 && (
      ...
      {addons.length > 0 && (
        <Field label="Add-ons" hint="Optional — pick as many as you'd like.">
          <div className="flex flex-col">
            {addons.map((a) => (
              <OptionRow
                key={a.id}
                selected={draft.addon_ids.includes(a.id)}
      ...
      <footer className="fixed inset-x-0 bottom-0 mx-auto w-full max-w-md border-t border-hairline bg-paper/95 px-[var(--screen-pad)] py-3 backdrop-blur">
        {estimate && (
          <div className="mb-2 flex items-baseline justify-between">
            ...
            <span className="text-sm font-semibold" style={{ color: "var(--wine-fg)" }}>
